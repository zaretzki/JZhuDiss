...
The hazard function baseline hazard for Weibull distribution with shape $\alpha$ and scale $\lambda$ is $h_0(t)=\alpha(\lambda)^\alpha t^{\alpha-1}$.  Extending this to a hazard from a proportional hazards regression model for $Age$, we simply multiply the hazard by the exponentiated linear predictor, which simply shifts the baseline up or down,
 ...
 \end{split}
 \end{equation}
 where, $\tilde{\lambda}=\lambda (exp(\beta \times Age)^{\frac{1}{\alpha}}$.  The survival times $T_i$ are randomly generated from the Weibull distribution with shape parameter $\alpha=1.5$ and $\tilde{\lambda}=exp(1.5 +0.025 \times Age)^{\frac{1}{\alpha}}$.  It follows that $\lambda = e^1$.
...
The simulations are performed using the {\tt coxreg} and {\tt phreg} functions from the {\tt R}-package {\tt eha} \citep{brostrom2012} for model fitting.  Function {\tt coxreg} performs a standard Cox proportional hazards regression using the partial likelihood.  The {\tt phreg} function performs a parametric proportional hazards regression using both Weibull, Extreme value (EV) baselines. 
