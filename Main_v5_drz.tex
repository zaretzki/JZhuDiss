...
For left truncation simulation, we again generate a sample of lifetimes $T_1, \ldots, T_n$.  For each observation, $i$, a uniform random variable $U_i \sim U(0,max(T))$ is then generated and represents the starting time. The stopping time $S_i = U_i+T_i$. Figure ... provides a histogram of the generated for $S$. Four levels of truncation are introduced by shifting the start of the sampling window, $L$ from 0 across the quartiles of $S$ to get 0\%, 25\%, 50\%, and 75\% truncation proportions. When the starting point $U_i$ for $ith$ observation is less than truncation time $l_i$, the starting point is reset as the truncation time $l_i$. [Need to discuss starting time and stop time notation used by Brostram.]

The results of the left truncation bias simulation statistics are shown on the right side of Table \ref{tab:rightcensor}. All the values shown are the average values over 100 replications of the simulation for coefficient estimates of age, and the baseline parameter estimates of $\lambda$ and $\alpha$ based on a parametric proportional hazards model assuming a Weibull distribution. 
%The last column is the total predicted number of events using non-parametric baseline by "coxreg" function. 
Similar to the earlier simulation results, the left truncation proportion and the number of events are two key factors for coefficients estimation. Table \ref{tab:rightcensor} indicates that, when the left truncation proportion is 75\% or the number of events are less than 200, the coefficients are overestimated. However, increasing the number of events can offset (reduce) the left truncation effects. For example, when the number of events is 1000 with 75\% truncation proportion, the estimates for age and $\alpha$ are all close to simulated value. 

Figure \ref{fig:lefttruncation} focuses on the number of events predicted by the model.  Here 2000 events are simulated with 100 replications for various proportions of left truncation. The red vertical line is the average of left truncation time. It is slightly overestimated using "coxreg" non-parametric baseline by four left truncation proportions, because the baseline estimation is not affected by the left truncation. Figure \ref{fig:leftbase} clearly shows the duration of non-parametric baseline are the same for four left truncation proportion. It also shows the non-parametric baselines (red lines) are overlap with parametric baseline (blue dash lines).
